...
The resource server **verifies** the signature using the corresponding public key. If the payload has been modified even by one character, the signature won't match and the token is rejected.`,
        ...
        },
        {
          ...
          questions: [
            {
              id: 'jwt1-q1',
              question: 'What are the three parts of a JSON Web Token, in order?',
              options: ['Signature.Payload.Header', 'Header.Payload.Signature', 'Payload.Header.Signature', 'Header.Signature.Payload'],
              correctIndex: 1,
              explanation: 'A JWT is Header.Payload.Signature, each Base64url-encoded and separated by dots. The header names the algorithm, the payload carries claims, and the signature verifies authenticity.',
            },
            {
              id: 'jwt1-q2',
              question: 'Why is it unsafe to trust the "alg" field from the JWT header when verifying a token?',
              options: [
                'The alg field is not Base64 encoded',
                'An attacker can set alg to "none" and strip the signature to forge a valid-looking token',
                'The algorithm field is optional in JWT',
                'Symmetric algorithms cannot be specified in the header',
              ],
              correctIndex: 1,
              explanation: 'The "alg: none" attack exploits libraries that read the algorithm from the token itself. Always hardcode the expected algorithm in your verification config — never trust the token to tell you how to verify it.',
            },
            {
              id: 'jwt1-q3',
              question: 'Which JWT claim specifies when the token expires?',
              options: ['iat (issued at)', 'nbf (not before)', 'exp (expiration time)', 'sub (subject)'],
              correctIndex: 2,
              explanation: '`exp` is a Unix timestamp after which the token must be rejected. `iat` is when the token was issued, `nbf` is the earliest valid time, and `sub` identifies the subject.',
            },
          ],
        },
      ],
          ...
          questions: [
            {
              ...
              options: [
                'The server\'s certificate is valid for future sessions',
                'Past sessions cannot be decrypted even if the server\'s long-term key is later stolen',
                'The client forwards its public key to the server for future use',
                'Secrets are shared in advance using a pre-shared key',
              ],
              correctIndex: 1,
              explanation: 'Forward secrecy (via ECDHE) means each session uses an ephemeral key pair. If the server\'s long-term private key is later compromised, previously recorded sessions remain encrypted because those ephemeral keys no longer exist.',
            },
            {
              id: 'https1-q2',
              question: 'What is the purpose of the ClientHello message in the TLS handshake?',
              options: [
                'To send the client\'s public certificate to the server',
                'To advertise the client\'s supported TLS versions and cipher suites',
                'To confirm the server\'s certificate is trusted',
                'To encrypt the session key with the server\'s public key',
              ],
              correctIndex: 1,
              explanation: 'ClientHello announces what the client supports — TLS versions, cipher suites, and extensions — so the server can choose the best shared configuration and respond with ServerHello.',
            },
            {
              id: 'https1-q3',
              question: 'Which TLS version made ECDHE key exchange mandatory?',
              options: ['TLS 1.0', 'TLS 1.1', 'TLS 1.2', 'TLS 1.3'],
              correctIndex: 3,
              explanation: 'TLS 1.3 removed older, weaker cipher suites and made ECDHE (ephemeral key exchange) mandatory, ensuring forward secrecy for all TLS 1.3 connections.',
            },
          ],
        },
      ],
  ...
  return stream; // call stream.cancel() to stop
        ...
        },
        {
          ...
          questions: [
            {
              id: 'grpc2-q1',
              question: 'Which gRPC streaming type allows both client and server to send multiple messages?',
              options: ['Unary RPC', 'Server-side streaming', 'Client-side streaming', 'Bidirectional streaming'],
              correctIndex: 3,
              explanation: 'Bidirectional streaming allows both client and server to independently send streams of messages over a single connection — useful for real-time scenarios like chat or live data feeds.',
            },
            {
              id: 'grpc2-q2',
              question: 'What is the correct gRPC status code to return when a requested resource does not exist?',
              options: ['INTERNAL', 'UNAVAILABLE', 'NOT_FOUND', 'INVALID_ARGUMENT'],
              correctIndex: 2,
              explanation: 'NOT_FOUND is the semantic match for a missing resource. INTERNAL signals an unexpected server-side failure unrelated to the request content, so using it for a 404 misleads the caller.',
            },
            {
              id: 'grpc2-q3',
              question: 'In a .proto file, which keyword defines a data type structure?',
              options: ['service', 'rpc', 'message', 'stream'],
              correctIndex: 2,
              explanation: '`message` defines a Protocol Buffers data type. `service` groups RPC methods, `rpc` declares a single method, and `stream` modifies an argument to indicate streaming.',
            },
          ],
        },
      ],
...
\`\`\``,
        ...
        },
        {
          ...
          questions: [
            {
              id: 'obs2-q1',
              question: 'What data collection model does Prometheus use?',
              options: [
                'Push — services send metrics to Prometheus',
                'Pull — Prometheus scrapes metrics from /metrics endpoints',
                'Stream — metrics arrive via WebSocket',
                'Batch — metrics are loaded from log files',
              ],
              correctIndex: 1,
              explanation: 'Prometheus uses a pull model: it periodically scrapes HTTP /metrics endpoints exposed by your services. This makes it straightforward to detect when a target goes down entirely.',
            },
            {
              id: 'obs2-q2',
              question: 'In the USE method, what does "Saturation" measure?',
              options: [
                'The error rate of a resource',
                'How much extra work is queued or waiting (overload indicator)',
                'The percentage of time a resource is actively busy',
                'The total throughput of a resource per second',
              ],
              correctIndex: 1,
              explanation: 'Saturation measures queued or deferred work — how much a resource is being asked to do beyond its capacity. High saturation means requests pile up even before utilization hits 100%.',
            },
            {
              id: 'obs2-q3',
              question: 'Which Prometheus metric type is best for tracking the total number of HTTP requests served?',
              options: ['Gauge', 'Counter', 'Histogram', 'Summary'],
              correctIndex: 1,
              explanation: 'A Counter only increases (or resets on restart). It\'s ideal for monotonically increasing values like request counts, error counts, or bytes sent. Use `rate()` in PromQL to get the per-second rate.',
            },
          ],
        },
      ],
...
Use **\`wss://\`** (WebSocket Secure, over TLS) in production for the same reasons you use HTTPS. Many proxies and CDNs will drop plain \`ws://\` connections.`,
        ...
        },
        {
          ...
          questions: [
            {
              id: 'ws1-q1',
              question: 'What HTTP mechanism initiates a WebSocket connection?',
              options: [
                'HTTP redirect (301 Moved Permanently)',
                'The Upgrade header with "Connection: Upgrade" and a 101 response',
                'HTTP CONNECT method tunneling',
                'A second TCP connection on a dedicated WebSocket port',
              ],
              correctIndex: 1,
              explanation: 'The client sends "Upgrade: websocket" and "Connection: Upgrade" in an HTTP/1.1 request. The server responds with 101 Switching Protocols, after which the TCP connection is used as a WebSocket — no HTTP framing anymore.',
            },
            {
              id: 'ws1-q2',
              question: 'After the WebSocket handshake, how are messages delimited on the wire?',
              options: [
                'HTTP Content-Length headers',
                'Newlines between messages',
                'Binary frames with opcode and payload length fields',
                'JSON envelope with a "type" field',
              ],
              correctIndex: 2,
              explanation: 'WebSocket defines its own binary framing format with opcodes (text, binary, ping, pong, close) and a payload length encoded in the frame header — completely independent of HTTP.',
            },
            {
              id: 'ws1-q3',
              question: 'Why do most WebSocket applications use JSON `{ type, payload }` messages?',
              options: [
                'The WebSocket specification requires JSON for text frames',
                'JSON is significantly faster than binary encoding',
                'It is a convention that makes routing different event types over one socket easy',
                'WebSocket cannot carry binary frames, only text',
              ],
              correctIndex: 2,
              explanation: 'JSON is purely a convention. WebSocket supports both text and binary frames. Using `{ type, payload }` envelopes lets a single socket carry many logical message types with a simple client-side switch.',
            },
          ],
        },
      ],
...
The server pings every 30 seconds. If a client doesn't pong within the next interval, it gets terminated — preventing ghost connections from accumulating.`,
        ...
        },
        {
          ...
          questions: [
            {
              id: 'ws2-q1',
              question: 'When broadcasting a message to all clients in a Node.js WebSocket server, what must you check before calling `ws.send()`?',
              options: [
                'That the client has an active HTTP session cookie',
                'That `ws.readyState === WebSocket.OPEN`',
                'That the message payload is under 1 MB',
                'That the client sent at least one message first',
              ],
              correctIndex: 1,
              explanation: 'A socket in `wss.clients` can be in CLOSING or CLOSED state. Calling `send()` on a non-OPEN socket throws an error. Always guard with `ws.readyState === WebSocket.OPEN` before sending.',
            },
            {
              id: 'ws2-q2',
              question: 'What reconnection strategy should a WebSocket client use after an unexpected disconnect?',
              options: [
                'Immediately retry in a tight loop',
                'Exponential back-off starting at ~1s and capping around 30s',
                'Wait exactly 60s between every reconnect attempt',
                'Reload the browser page',
              ],
              correctIndex: 1,
              explanation: 'Exponential back-off prevents a thundering herd where thousands of clients all reconnect simultaneously after a server restart. Double the delay each attempt with a maximum cap, and reset on a successful connection.',
            },
            {
              id: 'ws2-q3',
              question: 'In a multi-server deployment, why can\'t you broadcast by iterating a single server\'s client list?',
              options: [
                'WebSocket connections are stateless across servers',
                'Clients connected to other server instances won\'t receive messages broadcast from one process',
                'Node.js cannot hold multiple WebSocket connections simultaneously',
                'HTTP load balancers strip WebSocket upgrade headers',
              ],
              correctIndex: 1,
              explanation: 'Each server process only tracks its own connected clients. To reach clients on other instances you need a pub/sub layer (e.g., Redis Pub/Sub) so every server receives the message and forwards it to its own clients.',
            },
          ],
        },
      ],
